...
int main(int argc, char **argv){
	...
	for(i = 0; i < numtasks; i++){
		MPI_Irecv(&(tempMat[0][0]), chunkSize * cols, MPI_DOUBLE, prev, tagP, MPI_COMM_WORLD, &reqs[0]);
		MPI_Irecv(&(tempL[0]), chunkSize, MPI_INT, prev, tagL, MPI_COMM_WORLD, &reqs[2]);
		
		MPI_Isend(&(Yb[0][0]), chunkSize * cols, MPI_DOUBLE, next, tagP, MPI_COMM_WORLD, &reqs[1]);
		MPI_Isend(&(L[0]), chunkSize, MPI_INT, next, tagL, MPI_COMM_WORLD, &reqs[3]);
	...
	if(rank == MASTER){
		...
		printf("Time needed for knn search is %.4f sec\n", seq_time);
	}
	...
	if(rank == MASTER) {
		printf("Total match percentage is %.2f%%.\n", wholeMatch);
	}
